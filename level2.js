...
if(choice !== `1` && choice !== `2` ) {
 ...
 if (choice === `2`){
   ...
   let correctAnswers = 0;
    ...
    for(let y = 0; y < totalQuestions; y++ ){
        let uA = prompt(`Question ${y + 1}: What is ${questions[y].q}?`);
    let answer = Number(uA);

     if (isNaN(answer)) {
        console.log(`cant recognize that one twain. Score: ${score} + 0`);
        continue;
     }

    
    if (Math.abs(answer - questions[y].a) < 0.01) {
...
if (correctAnswers === 0){
    ...
    console.log (`the end.. Score: ${score} ${correctAnswers}`);
}
    ...
    if (Math.abs(answer - questions[x].a)< 0.01){
      ...
      console.log(`Question answer was:${questions[x].a}. Score: ${score} + 0`);
    }
